...
def tree_search(problem, fringe):
    ...
    count = 0
    fringe.append(Node(problem.initial)) # añadimos el problema inicial
    while fringe:
        node = fringe.pop()
        if problem.goal_test(node.state):
            print "Nodos expandidos", count
            return node
        fringe.extend(node.expand(problem),problem) #la heuristica
        count+=1
    return None
...
def rama_tree_search(problem):
    """Search en rama. """
    return tree_search(problem,OpenList()) #busqueda en árbol
